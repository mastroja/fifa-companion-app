...
do
    ...
    -- pcall'd around os.date — confirmed live 2026-08-30 in the TRANSFERS
    -- EXPORT block: a "date" field read from a negotiation record came
    -- back large/negative enough that os.date throws "date result cannot
    -- be represented in this installation" (a real dayOffset just parses
    -- fine; this is the record-parsing equivalent of a bad pointer — some
    -- field wasn't at the offset expected). That error was propagating out
    -- of convertFifaDate and wiping out the entire batch of records being
    -- built at the time, not just that one record's date. Every local copy
    -- of convertFifaDate in this file gets the same guard on the same
    -- reasoning — a single bad birthdate/join-date should cost that one
    -- date string, not the whole squad/youth/transfers export it's part of.
        ...
        while current_record > 0 do
            ...
            if playerid and playerid > 0 then
                ...
                if is_user_team or is_loaned_out then
                    ...
                    -- 1 (palest) - 10 (darkest) skin tone lightness value,
                    -- used client-side to bucket which local headshot photo
                    -- to show (no real in-game player photo source exists)
                    -- — see resolveHeadshotPath in main.js and the players
...
do
    ...
    -- 20250706 = July 6, 2025). Running it through convertFifaDate
    -- computed a nonsense far-future timestamp that always failed the
    -- os.date pcall guard above, which is why every deal_date has been
    -- blank since this feature shipped. Confirmed via a live memory probe
    -- (2026-08-31): raw struct bytes compared against GetCurrentDate()
    -- showed this exact YYYYMMDD pattern consistently across every
    -- succeeded transfer and loan negotiation sampled.
        ...
        walk_negotiation_vector(storage, 0x20, 0x98, function(current)
            ...
            if (playerid > 0 and buying_team > 0 and selling_team > 0) then
                ...
                if (seller_accepted) then
                    ...
                    -- this negotiation's fixed inline array (slot 0 is
                    -- always an unused -1 sentinel) — indexing it directly
                    -- lands on the FINAL action (the one that actually
                    -- completed the deal). The "-1" this used to subtract
                    -- landed one slot too early, on an offer/counter
                    -- instead — confirmed via a live memory probe
                    -- (2026-08-31) comparing raw struct bytes against
                    -- GetCurrentDate(): every sampled loan showed its true
                    -- final action (the one whose action-type byte at +0x8
                    -- reads 0, same "completed" marker the pointer-based
                    -- user_player_transfer/exchange functions already key
                    -- off of) sitting one slot AFTER what the old "-1" math
                    -- picked.
...
do
    ...
    -- pcall'd around os.date for the same reason as the other copies (see
    -- the TRANSFERS EXPORT block's convertFifaDate) — a bad dayOffset
    -- should cost one date string, not abort this whole export.
    local function convertFifaDate(dayOffset)
        if not dayOffset or dayOffset <= 0 then return "" end
        local baseEpochSeconds = -12219292800
        local targetSeconds = baseEpochSeconds + (dayOffset * 86400)
        local ok, formatted = pcall(os.date, "%Y-%m-%d", targetSeconds)
        if ok and formatted then return formatted end
        return ""
    end
